...
function add-onboardScript {
    ...
    # Download the script
    $download = get-download -Url "$url/$subPath/$script.ps1" -Target "$env:SystemRoot\Temp\$script.ps1" -failText "$url/$subPath/$script.ps1Could not acquire onboarding components."
    if (!$download) { 
        read-command 
    }
