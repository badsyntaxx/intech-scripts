function installBGInfo {
    try {
        ...
        $download = getDownload -Url $url -Target "$env:SystemRoot\Temp\$target`_BGInfo.zip" -label "Downloading BGInfo"
        ...
        writeText -type "error" -text "install-bginfo-$($_.InvocationInfo.ScriptLineNumber) | $($_.Exception.Message)"
    }
}
