function isr-set-audio-device {
    try {
        # Get audio devices with separate selections
        $devices = Get-PnpDevice | Where-Object { $_.Class -eq "AudioEndpoint" }

        $orderedDevices = [ordered]@{}

        foreach ($d in $devices) {
            $orderedDevices[$d.FriendlyName] = $d.InstanceId
        }
        
        $device = read-option -options $orderedDevices -returnKey
        ...
        exit-script -type "error" -text "isr-set-audio-device-$($_.InvocationInfo.ScriptLineNumber) | $($_.Exception.Message)" -lineAfter
    }
}
...
invoke-script 'isr-set-audio-device'